...
我们故意损坏第一字符的第二和第三字节,因此第一字符将会打印"?",第二和第三字节则被忽略,后面的"abc"依然可以正常解码打印:
```go
...
用copy和append组合也可以实现在中间位置插入多个元素:
```go
...
其实类似的根据过滤原地删除切片元素的算法都可以采用类似的方式处理:
```go
...
下面的代码通过2种方法将`[]float64`类型的切片转换为`[]int`类型的切片:
```go
// +build amd64 arm64
